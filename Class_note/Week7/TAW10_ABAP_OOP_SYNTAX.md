...
&nbsp;&nbsp;&nbsp;&nbsp;P.83  
    오브젝트를 생성하는 특별한 메쏘드를 컨스트럭터 라고 한다.  
명시적으로 호출 되지 않는다?  
클래의 오브젝트가 생성될때 자동적으로 컨스트럭터로 생성된다  
  
- 컨스트럭터는 IMPORTING과 EXCEPTION만 가질 수 있다.  
  
- METHOD 이름은 `CONSTRUCTOR` 라는 이름만 와야한다!!!!!!!!!!!!!!!!!!  
  
- PUBLIC-SECTION에만 선언해야한다.  
  
- 클래스당 1개의 CONSTRUCTOR만 올 수 있다.  
...
컨스트럭터로 호출했으나 기준에 맞지않아 오브젝트가 생성되지 않았는데
생성되지 않은 오ㅡ젝트를 호출하려고하면 Short Dump가 발생한다.
